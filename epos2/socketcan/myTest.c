...
typedef struct
{
	...
	CMSG		*obj[CMSG_SIZE]; // pointer  array of CMSG 
} NTCAN;                                 // structs
...
int32_t initNTCAN(uint32_t baud,  uint32_t flags,
		 int32_t ID[],   int32_t idLen, int32_t net,
...
/* Adding an ID */
   ...
   printf("Initializing sucessfull, system ready to use\n");
   return handle; 
}
...
/* Deallocating memory from myNTCAN */
