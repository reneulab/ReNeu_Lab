/*
 * FPGA Interface C API 12.0 header file.
 *
 * Copyright (c) 2012,
...
/**
 * LabVIEW FPGA does not support the Reset method for bitfiles that allow the
...
/**
 * LabVIEW FPGA does not support the Abort method for bitfiles that allow the
 * removal of implicit enable signals in single-cycle Timed Loops.
 */
 ...
 * that allow the removal of implicit enable signals in single-cycle Timed
 * Loops. Pass the NiFpga_CloseAttribute_NoResetIfLastSession attribute to
 * NiFpga_Close instead of 0.
 */
static const NiFpga_Status NiFpga_Status_CloseAndResetCalledWithImplicitEnableRemoval = -61213;

/**
 * For bitfiles that allow the removal of implicit enable signals in
 * single-cycle Timed Loops, LabVIEW FPGA does not support this method prior to
 * running the bitfile.
 */
static const NiFpga_Status NiFpga_Status_ImplicitEnableRemovalButNotYetRun = -61214;

/**
 * Bitfiles that allow the removal of implicit enable signals in single-cycle
 * Timed Loops can run only once. Download the bitfile again before re-running
 * the VI.
 */
