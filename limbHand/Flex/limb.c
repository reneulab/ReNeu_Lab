...
int32_t moveLimb(NiFpga_Session session, arg *myArg)
{
	...
	int32_t count=0;
	command myCmd;
	NTCAN_HANDLE myHandle;
	myCmd = myArg->cmd;
	myHandle = myArg->handle;
	desired = myArg->initAngle - myArg->angle;

	do 
	{	lastAngle = realAngle;
		result = writeLimb(myHandle,&myCmd);
  		if(result != 0)
  			{ return 1; }
		NiFpga_ReadI32(session,myArg->flex,&realAngle); 
		if(realAngle > desired)
			{ myCmd.mode = l_close; }
		else if(realAngle < desired)
			{ myCmd.mode = l_open; }
		if(realAngle==lastAngle)
		{ 
			count++;
			if(count > 20)
			{ break; }
	...
	} while( (realAngle > (desired+5) ) || (realAngle < (desired-5))); 

	usleep(20000);	
	myCmd.mode = l_stop;
	writeLimb(myHandle,&myCmd);	
	printf("Digit: %d Angle: %d Desired: %d\n", myArg->digit, realAngle,desired);
	return 0; 
}
